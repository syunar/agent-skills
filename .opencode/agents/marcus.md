...
Require `builder` to run assigned verification and report results before review. Then delegate implementation review to `reviewer`; only verdict `ship` passes. For any other verdict, delegate exact findings to `feedback-responder`, verify fixes with focused checks, then re-run `reviewer`; repeat until `ship`. Stop and report a blocker if a finding cannot be resolved, verification persistently fails, or delegation fails. Do not commit, push, or open a PR before `ship`; pushes and PRs still require explicit user request.
